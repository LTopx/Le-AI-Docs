...
- Le-AI will never automatically sync any of your information. This includes but is not limited to session lists, configuration information, and various API keys.
- All backup and restore operations in Le-AI can only be triggered manually, and there are no plans to develop automatic backup and restore functionality in the future.
- All data is stored locally on the browser and device by default, and no one else except yourself can access this data.
- When using the backup and cloud sync feature, Le-AI will encrypt the data locally with an AES encryption key provided by you before uploading it to the cloud. The same key is required for decryption during data restoration.
...
- Free users: There is a free backup quota of 50KB. Backup will not be successful if it exceeds the quota.
- Premium and Team users: They have a backup quota of 1M. The quota may be adjusted or separate quota packages may be offered based on user feedback in the future.
